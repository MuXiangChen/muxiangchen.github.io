<template><div><h2 id="浏览器的事件循环机制-重要" tabindex="-1"><a class="header-anchor" href="#浏览器的事件循环机制-重要"><span>浏览器的事件循环机制（重要）</span></a></h2>
<p><img src="https://img.smyhvae.com/image-20230608154453933.png" alt="image-20230608154453933"></p>
<p>执行顺序如下：</p>
<ul>
<li>
<p>同步任务：进入主线程后，立即执行。</p>
</li>
<li>
<p>异步任务：会先进入 Event Table；等时间到了之后，再进入 任务队列 （Event Queue）排队（排队是因为同一时间，JS 只能执行一个任务），先进先出。比如说，<code v-pre>setTimeout(()=&gt; {}, 1000)</code>这种定时器任务，需要等一秒之后再进入 Event Queue。</p>
</li>
<li>
<p>当主线程的任务执行完毕之后，此时主线程处于空闲状态，于是会去读取 Event Queue 中的任务队列，如果有任务，则进入到主线程去执行。</p>
</li>
</ul>
<h2 id="node-js-事件循环机制" tabindex="-1"><a class="header-anchor" href="#node-js-事件循环机制"><span>Node.js 事件循环机制</span></a></h2>
<p>浏览器的 EventLoop 依据的是 HTML5 规范。而 Node.js 的 EventLoop 是由Node.js底层的 libuv 规定的。 libuv是一个专注于异步IO的跨平台库。</p>
<p>Node.js的事件循环中，有六个队列。其中，微任务有两个队列，宏任务有四个队列。</p>
<p>一、微任务队列：</p>
<ul>
<li>顺序1：next tick queue。比如：process.nextTick</li>
<li>顺序2：other queue。比如：Promise的then回调、queueMicrotask</li>
</ul>
<p>二、宏任务队列：</p>
<ul>
<li>顺序3：timer queue。比如：setTimeout、setInterval</li>
<li>顺序4：poll queue。比如：IO事件</li>
<li>顺序5：check queue。比如：setImmediate</li>
<li>顺序6：close queue。比如：close事件</li>
</ul>
<p>参考链接：</p>
<ul>
<li>【荐】浏览器及nodeJS中的EventLoop事件循环机制：https://www.cnblogs.com/weiyongchao/p/13766429.html</li>
<li>浏览器和Node 事件循环的区别：https://github.com/Advanced-Frontend/Daily-Interview-Question/issues/26</li>
</ul>
<h2 id="宏任务和微任务" tabindex="-1"><a class="header-anchor" href="#宏任务和微任务"><span>宏任务和微任务</span></a></h2>
<p>JS中的任务分为同步任务、异步任务。</p>
<p>JS中的异步任务分为宏任务（macrotask）、微任务（microtask）。在早期，异步任务中只有宏任务，没有微任务。后来的语言标准中，推出了“微任务”，因为<strong>希望微任务能够尽早执行</strong>。</p>
<h3 id="宏任务、微任务分类" tabindex="-1"><a class="header-anchor" href="#宏任务、微任务分类"><span>宏任务、微任务分类</span></a></h3>
<p>事件循环的队列中，有两个队列。</p>
<p>1、<strong>宏任务队列</strong>，包含这些任务：</p>
<ul>
<li>ajax 网络请求</li>
...
<p>2、<strong>微任务队列</strong>，包含这些任务：</p>
<ul>
<li>Promise的then回调</li>
<li>Mutation Observer API：监听DOM节点变化。</li>
<li>queueMicrotask()：可直接将某个任务加入到微任务队列中。</li>
</ul>
<p>在执行一个 Promise 对象时，当走完 resolve() 进入 fulfilled状态后，会立刻把 <code v-pre>.then()</code>里面的代码加入到<strong>微任务队列</strong>当中。</p>
<h3 id="任务的执行顺序" tabindex="-1"><a class="header-anchor" href="#任务的执行顺序"><span>任务的执行顺序</span></a></h3>
<p>JS中的任务执行顺序：<strong>同步任务 --&gt; 微任务 --&gt; 宏任务</strong>。</p>
<p>在执行任何一个宏任务之前（不是队列，是一个宏任务），都会<strong>先查询微任务队列中是否还有任务需要执行</strong>：</p>
<ul>
<li>当前宏任务执行之前，必须要保证微任务队列是空的。</li>
<li>如果微任务队列不为空，那就优先执行微任务队列中的任务。</li>
</ul>
<h2 id="任务执行顺序的面试题" tabindex="-1"><a class="header-anchor" href="#任务执行顺序的面试题"><span>任务执行顺序的面试题</span></a></h2>
<p>实际开发中，基本不会出现下面这些题目，因为很多时候我们无法精准控制异步任务的执行顺序。但是它们在面试中出现的频率特别高，因为熟悉这些思维训练，有利于考察我们对JS单线程、事件循环机制、宏任务和微任务等原理的掌握程度。</p>
<h3 id="题-1-宏任务和微任务的执行顺序" tabindex="-1"><a class="header-anchor" href="#题-1-宏任务和微任务的执行顺序"><span>题 1：宏任务和微任务的执行顺序</span></a></h3>
...
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>上方代码执行的顺序依次是：<strong>同步任务 --&gt; 微任务 --&gt; 宏任务</strong>。</p>
<h3 id="题-2-在宏任务中嵌套了微任务" tabindex="-1"><a class="header-anchor" href="#题-2-在宏任务中嵌套了微任务"><span>题 2：在宏任务中嵌套了微任务</span></a></h3>
...
<span class="line">promise then</span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>上方代码解释：在执行宏任务的<strong>过程中</strong>，创建了一个微任务。但是需要<strong>先把当前这个宏任务执行完</strong>，再去<strong>创建并执行</strong>微任务。</p>
<h3 id="题3-综合题" tabindex="-1"><a class="header-anchor" href="#题3-综合题"><span>题3：综合题</span></a></h3>
<div class="language-javascript line-numbers-mode" data-highlighter="prismjs" data-ext="js" data-title="js"><pre v-pre><code><span class="line">console<span class="token punctuation">.</span><span class="token function">log</span><span class="token punctuation">(</span><span class="token string">"script start"</span><span class="token punctuation">)</span></span>
<span class="line"></span>
<span class="line"><span class="token function">setTimeout</span><span class="token punctuation">(</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token operator">=></span> <span class="token punctuation">{</span></span>
...
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>打印结果：</p>
<div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text" data-title="text"><pre v-pre><code><span class="line">// 第一次循环</span>
...
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="题4-async-await-题目" tabindex="-1"><a class="header-anchor" href="#题4-async-await-题目"><span>题4：async await 题目</span></a></h3>
<p>代码举例：</p>
<div class="language-javascript line-numbers-mode" data-highlighter="prismjs" data-ext="js" data-title="js"><pre v-pre><code><span class="line">console<span class="token punctuation">.</span><span class="token function">log</span><span class="token punctuation">(</span><span class="token string">'script start'</span><span class="token punctuation">)</span></span>
<span class="line"></span>
